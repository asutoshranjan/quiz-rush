                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {data.transactions.map((transaction: any, index: number) => (
                            ...
                            >
                                <div className="mb-2">
                                    <p className="text-sm font-medium text-gray-600">Txn Signature:</p>
                                    <p className="text-gray-800 break-all">{transaction.txnSignature}</p>
                                </div>
                                <div className="flex justify-between">
                                    <p className="text-sm text-gray-600">
                                        <span className="font-medium">Amount:</span> {transaction.amount/1000_000_000} SOL
                                    </p>
                                    <p className="text-sm text-gray-600">
                                        <span className="font-medium">Status: </span> 
                                        <span className={`${transaction.status == "paid" ? "text-green-600" : ""}`}>{transaction.status}</span>
                                    </p>
                                </div>
